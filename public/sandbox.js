(() => {
  ...
  document.addEventListener('computedValueChanged', function () {
    var value = window.computedValue;
    outChannel.postMessage({ value })
});

  const innerScript = (code) => {
    return `window.computedValue = (function(){ ${code} })(); document.dispatchEvent(new Event('computedValueChanged'));`
  }
  ...
  addEventListener("message", e => {
    ...
    if (e.data.type == "load") {
      ...
      tag.type = "module"
      tag.textContent = innerScript(e.data.code)
